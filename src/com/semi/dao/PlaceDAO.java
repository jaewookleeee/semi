...
public class PlaceDAO {
	...
	public long write(DTO dto) {
		String sql = "INSERT INTO place(place_no,info_id,place_name,place_intro,place_home,"
				+ "place_loc,place_guid,place_attention,place_date,place_category,place_phone)"
				+ " VALUES(place_seq.NEXTVAL,?,?,?,?,?,?,?,SYSDATE,?,?)";
		long success = 0;
		try {
	...
	public void photowrite(ArrayList<DTO> list, long success) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (success > 0) {
				sql = "INSERT INTO photo (place_no,place_photo)" + "VALUES(?,?)";
				try {
					ps = conn.prepareStatement(sql);
					ps.setLong(1, success);
